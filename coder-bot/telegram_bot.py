...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with _processing_lock:
        if _processing:
            # Проверяем не зависла ли обработка
            if _processing_started > 0 and (time.monotonic() - _processing_started) > PROCESSING_HARD_RESET:
                log.error("Force-resetting stale _processing lock in handle_message")
                _processing = False
            else:
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global _processing, _processing_started
    if not is_allowed(update):
        return

    async with _processing_lock:
        if _processing:
            # Проверяем не зависла ли обработка (как в handle_message)
            if _processing_started > 0 and (time.monotonic() - _processing_started) > PROCESSING_HARD_RESET:
                log.error("Force-resetting stale _processing lock in handle_photo")
                _processing = False
                _processing_started = 0.0
            else:
                await update.message.reply_text("Ещё обрабатываю предыдущий запрос, подожди.")
    ...
    try:
        ...
        _ticker_task = asyncio.create_task(_thinking_ticker(thinking_msg, _t_start, prefix="📸 Обрабатываю фото"))
        photo = update.message.photo[-1]
        file = await context.bot.get_file(photo.file_id)
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        photo_path = PHOTO_DIR / f"photo_{ts}_{photo.file_id[-8:]}.jpg"
        # Скачивание фото С ТАЙМАУТОМ — без этого бот мог зависнуть навсегда!
        try:
