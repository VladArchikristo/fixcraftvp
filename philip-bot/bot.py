...
SYSTEM_PROMPT = (
    ...
    "  Если пользователь просит промт для маркетинга и нужен её экспертный контекст — упомяни что можно спросить её напрямую.\n\n"
    "• Василий (@vasily_trader_bot) — трейдер.\n"
    "  Если промт касается трейдинга/финансов — Василий знает контекст рынков.\n\n"
